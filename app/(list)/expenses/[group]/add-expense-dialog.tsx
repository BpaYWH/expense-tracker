...
function MutationDialog({ baseData, mutator, triggerText, header }: Props): JSX.Element {
	const [isKeepRepeatedFields, setIsKeepRepeatedFields] = React.useState<boolean>(false);
	...
	const resetForm = (): void => {
	   if (isKeepRepeatedFields) {
			form.resetField("item");
			form.resetField("price");
			form.resetField("taxRate");
			form.resetField("consumedUsers");
			return;
		}
		form.reset();
	};
	...
	const handleSubmit = (submitData: z.infer<typeof FormSchema>): void => {
		mutator({
			variables: {
				...submitData,
						...
						>
											...
											render={({ field }) => {
												return (
													<FormItem className="flex flex-row items-start space-x-3 space-y-0">
														<FormControl>
															<Select onValueChange={field.onChange} value={form.getValues("shopId")}>
																<FormControl>
																	<SelectTrigger>
																		<SelectValue />
																	</SelectTrigger>
										...
										<FormField
											...
											render={({ field }) => {
												return (
													<FormItem className="flex flex-row items-start space-x-3 space-y-0">
														<FormControl>
															<Select onValueChange={field.onChange} value={form.getValues("categoryId")}>
																<FormControl>
																	<SelectTrigger>
																		<SelectValue />
																	</SelectTrigger>
																...
																<SelectContent>
																	{baseData.categories.categories.map(
																		(category: any) => (
																			<SelectItem
										...
										<FormField
											...
											render={({ field }) => {
												return (
													<FormItem className="flex flex-row items-start space-x-3 space-y-0">
														<FormControl>
															<Select onValueChange={field.onChange} value={form.getValues("userId")}>
																<FormControl>
																	<SelectTrigger>
																		<SelectValue />
																	</SelectTrigger>
							...
							<div className="flex justify-between">
								<div className="flex items-center space-x-2">
									<Checkbox id="isKeepRepeatedFields" checked={isKeepRepeatedFields} onCheckedChange={() => {setIsKeepRepeatedFields(!isKeepRepeatedFields)}} />
									<label htmlFor="isKeepRepeatedFields" className="text-xs">Keep Shop, Categor, Paid By, Paid Date</label>
								</div>
