...
function ActionsCell({ id, item, price, taxRate, shop, paidAt, paidUser, consumedUsers, group, category, baseData, updator, deleter }: Props): JSX.Element {
    ...
    <div className='flex gap-4'>
		<Dialog>
			<DialogTrigger asChild>
				<Button variant="ghost" className="text-xs sm:text-sm">Edit</Button>
			</DialogTrigger>
                                    ...
                                    render={({ field }) => {
                                       return (
                                          <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                             <FormControl>
                                                <Select onValueChange={field.onChange} defaultValue={form.getValues("shopId")}>
                                                   <FormControl>
                                                      <SelectTrigger>
                                                      <SelectValue />
                                                      </SelectTrigger>
                                                   ...
                                                   <SelectContent>
                                                      {
                                                         (baseData.shopsData).shops.map((shop: any) => (
                                          ...
                                          <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                             <FormControl>
                                                <Select onValueChange={field.onChange} defaultValue={form.getValues("categoryId")}>
                                                   ...
                                                   <SelectContent>
                                                      {
                                                         (baseData.categoriesData).categories.map((category: any) => (
                                 ...
                                 <FormField
                                    ...
                                    render={({ field }) => {
                                       return (
                                          <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                                             <FormControl>
                                                <Select onValueChange={field.onChange} defaultValue={form.getValues("userId")}>
                                                   <FormControl>
                                                      <SelectTrigger>
                                                         <SelectValue />
                                                      </SelectTrigger>
                           ...
                           <Popover>
                              <PopoverTrigger asChild>
                                 <FormControl>
                                    ...
                                    >
                                       {field.value ? (
                                          format(fromUnixTime(Number(field.value) / 1000), "yyyy-MM-dd")
                                       ) : (
      ...
      <Dialog>
            ...
            <DialogFooter>
               <DialogTrigger asChild>
                  <Button variant="destructive" type="submit" onClick={handleDelete} disabled={confirmName !== item}>Delete</Button>
               </DialogTrigger>
            </DialogFooter>
			</DialogContent>
		</Dialog>
